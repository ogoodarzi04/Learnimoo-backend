...
exports.create = async (req, res, next) => {
   try {
      ...
      const { name, description, shortName, categoryID, price, support, status, discount } = req.body;
      ...
      const populatedCourse = await courseModel.findById(course._id).populate("creator", "-password");
...
exports.update = async (req, res, next) => {
   try {
      const updatedCategory = await categoryModel.findOneAndUpdate({ _id: req.params.id }, { name: req.body.name, title: req.body.title }, { new: true });
      if (!updatedCategory) {
...
exports.update = async (req, res, next) => {
   try {
      ...
      const { name, description, shortName, categoryID, price, support, status, discount, isComplete } = req.body;
      ...
      const newCourse = await courseModel.findById(oldCourse._id).populate("creator", "-password");

      if (oldCourse.cover !== newCourse.cover) {
         const imgPath = path.join(__dirname, "..", "..", "public", "courses", "covers", oldCourse.cover);
...
exports.getAll = async (req, res, next) => {
   try {
      const courses = await courseModel.find().populate("creator", "-password").populate("categoryID").lean().sort({ _id: -1 });
      ...
      courses.forEach((course) => {
         ...
         let courseScores = comments.filter((comment) => comment.course.toString() === course._id.toString());
...
exports.getOne = async (req, res, next) => {
   try {
      ...
      const course = await courseModel.findOne({ shortName: req.params.shortName }).populate("categoryID", "-password").populate("creator", "-password").lean();
      if (!course) {
         return res.status(404).json({ message: "Course Not Found!" });
      }

      const sessions = await sessionModel.find({ course: course._id }).lean();
      const comments = await commentModel.find({ course: course._id, answer: 1 }).populate("creator", "-password").lean();
...
exports.getAllSessions = async (req, res, next) => {
   try {
      const allSessions = await sessionModel.find().populate("course", "name").lean();
...
exports.register = async (req, res, next) => {
   try {
      ...
      if (isUserAlreadyRegistered) {
         return res.status(409).json({ message: "you are already registered to this course." });
      }
...
exports.removeSession = async (req, res, next) => {
   try {
      ...
      const videoPath = path.join(__dirname, "..", "..", "public", "courses", "covers", deletedSession.video);
...
exports.getSessionInfo = async (req, res, next) => {
   try {
      ...
      const course = await courseModel.findOne({ shortName: req.params.shortName }).lean();
